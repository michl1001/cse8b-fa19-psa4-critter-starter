...
class CritterManager extends Observable{
	// Instance variables useful for fancy operations
	protected final List<Critter> critterList;
	...
	public synchronized boolean addBuff(){
		...
		for (int i = 0; i < critterList.size(); i++){
			...
			// Perform some buff every 25 moves

			if (moveCount % buffTurn == 1){
				c.buffBehavior();
				extraEffect = true;
			}
			if (moveCount % buffTurn == 2){
				c.debuff();
			}

			if (moveCount % 15 == 1 && c instanceof Starfish){
				((Starfish) c).teleport(getLocation(c), grid);
			}
		}
	...
	public CritterState getState (Critter c){
		...
		if (c instanceof Turtle || c instanceof Ocelot || c instanceof Elephant || c instanceof Starfish){
            return classStateMap.get(c.getClass().getName());
        }
